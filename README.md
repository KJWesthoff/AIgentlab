...
| Module | Responsibility |
|---|---|
| `workflow.py` | `Workflow.execute(objective)` — the deterministic pipeline: researcher → analyst → writer → reviewer, then at most `MAX_REVISIONS = 1` rewrite if the reviewer rejects **and says what to change** — a rejection naming no changes stops the run rather than re-running write/review on identical input. Returns a `WorkflowResult` bundling the final answer, all intermediate artifacts, the approval status and the budget totals. The routing is ordinary Python — no tokens are spent asking a supervisor what to do next. |
...
sequenceDiagram
    ...
    alt review.approved == false, with notes (max 1 revision)
        W->>R: run_text(writer, + reviewer notes + previous draft)
...
ReviewResult { approved, required_changes, unsupported_statements }
    ...
    └─ rejected ─► writer (once, with the reviewer's notes) ─► reviewer ─► final answer
       (rejected with no notes ─► stop: a revision with nothing to act on
        would hand the writer the input it just answered)
```
